...
import java.util.HashMap;
    ...
    public class Server {
        ...
        //private BufferedInputStream[] is;//写真よう
        ...
        public Server(int port) { //待ち受けポートを引数とする
            ...
            //is = new BufferedInputStream[maxconnection];
        ...
        class Receiver extends Thread {
            ...
            Receiver(Socket socket, int playerNo) {
                ...
                try {
                    ...
                    is[playerNo] = new BufferedInputStream(socket.getInputStream());
            ...
            public void run() {
                ...
                try {
                    ...
                    while(true) {// データを受信し続ける
                        ...
                        if(inputLine.equals("立候補取り消し")) {
						//a=br[playerNo].readLine();//質問内容の受信

						StringBuffer buf = new StringBuffer();

                        while(!(a = br[playerNo].readLine()).equals("finish")) {
                            buf.append(a+"\n");
                        }
                        forwardMessage("読み込み終了", playerNo);
                        int num = buf.lastIndexOf("\n");
                        int last = buf.length();
                        buf = buf.delete(num,last);
                        String k = new String(buf);

						String name=hashA1.get(playerNo);
						Question q=ques(k);
						q.delCandidates(obj(name));
						qnew(q);
                        ...
                        if(inputLine.equals("質問内容")) {
                            ...
                            byte[] getbmp = null;//写真保持用
                            if(br[playerNo].readLine().equals("あり")) {
                            	int n = Integer.parseInt(br[playerNo].readLine());
                                isImage = true;
                                forwardMessage("OK", playerNo);
                                while(is[playerNo].available() == 0) {

                                }
                                //写真用
                                if(is[playerNo].available() != 0) {
                                    //forwardMessage("ImReady",playerNo);
                                    System.out.println(is[playerNo].available());

                                    getbmp=new byte[n];//長さ分生成
                                    int count = 0;
                                    while(n > 0) {
                                    	int done = is[playerNo].read(getbmp,count,n);
                                    	n -= done;
                                    	count += done;
                                    }
                                    //is[playerNo].read(getbmp,0,is[playerNo].available());
                                    //
                                    System.out.println(getbmp.length);
                                    System.out.println(is[playerNo].available());

                                    //BufferedImage img = F(getbmp);
                                    //ImageIO.write(img,"png",new File("hello2.png"));

                                    System.out.println("書込完了");
                                    out[playerNo].println("書込完了");
                                    out[playerNo].flush();
                            ...
                            Question p=new Question(ques,k,group,coin);
                            if(isImage) {//写真が添付されているならば
                                p.setImage(getbmp);
                            }
                            p.setIsImage(isImage);
                            Group g=chat(group);
                            g.setchat(p);
                        ...
                        if(inputLine.equals("回答立候補")) {
                            ...
                            buf = buf.delete(num,last);
                            forwardMessage("読み込み終了", playerNo);
                            String questr = new String(buf);

                            String name=hashA1.get(playerNo);

                            User er=obj(name);
                            Question k=ques(questr);
                            if(k!=null) {
                                k.setCandidates(er);
                        ...
                        if(inputLine.equals("オファー来てるかな")) {
                            String name=hashA1.get(playerNo);
                            System.out.println(name + "さんへのオファーを取り出す");
                        ...
                        if(inputLine.equals("オファー人選出")) {
                            ...
                            while(!(b = br[playerNo].readLine()).equals("finish")) {
                                buf.append(b+"\n");
                            }
                            int num = buf.lastIndexOf("\n");
                            int last = buf.length();
                            ...
                            System.out.println(a + "さんへのオファーを受け取りました");
                            Question k = ques(questr);
                            if(k!=null) {
                                k.setOffer(obj(a));
                        ...
                        if(inputLine.equals("評価値変更")) {
                            ...
                            //String name=hashA1.get(playerNo);
                            String name=q.getAnswerer().getName();
        ...
        public static void main(String[] args) { //main
            ...
            Server server = new Server(10031); //待ち受けポート10000番でサーバオブジェクトを準備
            ...
            try {
                ...
                User u2 = new User("学生1","大学生","理系" );
                outObject.writeObject(u);
                outObject.writeObject(u2);
                outObject.close();
                outFile.close();
                ...
                Group g2 = new Group("横浜国立大学", "横浜国立大学についてお話しましょう", u2);
                outObject1.writeObject(g);
                outObject1.writeObject(g2);
                outObject1.close();
                outFile1.close();
